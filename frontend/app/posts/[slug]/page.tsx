...
export default async function PostPage(props: Props) {
  ...
  const readingTime = calculateReadingTime(post.content as PortableTextBlock[])
            ...
            <div className="pb-6 grid gap-6 mb-6 border-b border-gray-100">
              <div className="max-w-3xl flex flex-col gap-6">
                <h1 className="text-4xl text-gray-900 sm:text-5xl lg:text-7xl">
                  {post.title && <FlickerText text={post.title} />}
                </h1>
              </div>
              ...
              <div className="relative">
                {post?.coverImage && (
                  <>
                    ...
                    />
                    <div className="absolute inset-x-0 bottom-0 flex flex-wrap gap-2 bg-black/70 p-4 text-xs font-mono text-white backdrop-blur-sm">
                      {post.category && (
                        <span className="border border-white/20 bg-white/10 px-2 py-1">
                          {post.category}
                        </span>
                      )}
                      {post.difficulty && (
                      ...
                      <span className="border border-white/20 bg-white/10 px-2 py-1">
                        czas czytania: {readingTime} minut
                      </span>
                      {post.date && (
                        <time className="border border-white/20 bg-white/10 px-2 py-1" dateTime={post.date}>
                          data publikacji: <DateComponent dateString={post.date} />
                        </time>
                      )}
                    </div>
                  </>
