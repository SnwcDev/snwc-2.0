...
export type Post = {
  ...
  difficulty: 'poczatkujacy' | 'sredni' | 'srednio-zaawansowany' | 'zaawansowany'
  category: string
  date: string
...
// Query: *[_type == "post" && defined(slug.current)] | order(date desc, _updatedAt desc) {      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  coverImage,  difficulty,  category,  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
...
// Query: *[_type == "post" && defined(slug.current)] | order(date desc, _updatedAt desc)[0...3] {    _id,    "title": coalesce(title, "Untitled"),    "slug": slug.current,    coverImage,    difficulty,    category,    "date": coalesce(date, _updatedAt),    content  }
